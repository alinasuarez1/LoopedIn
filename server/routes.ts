...
export function registerRoutes(app: Express): Server {
    ...
    app.post("/api/webhooks/twilio", async (req, res) => {
        try {
            ...
            // Get all media URLs (Twilio sends them as MediaUrl0, MediaUrl1, etc.)
            const mediaUrls = Object.keys(req.body)
                .filter(key => key.startsWith('MediaUrl'))
            ...
            if (!user) {
                console.warn(`Received message from unknown number: ${From}`);
                return res.type('text/xml').send(`
            <?xml version="1.0" encoding="UTF-8"?>
            <Response>
              <Message>Sorry, we couldn't find your account. Please sign up first!</Message>
            </Response>
          `);
            }
            ...
            <?xml version="1.0" encoding="UTF-8"?>
            <Response>
              <Message>You're not a member of any loops yet. Ask your loop admin to add you!</Message>
            </Response>
          `);
            }
            ...
            <?xml version="1.0" encoding="UTF-8"?>
            <Response>
              <Message>Loop "${loopNameMatch[1]}" not found. Available loops: ${userLoops.map(m => m.loop!.name).join(', ')}</Message>
            </Response>
          `);
            }
            ...
            const savedUpdates = await Promise.all(
                targetLoops.map(async membership => {
                    // Create a base update with the text content
                    const baseUpdate = {
                        loopId: membership.loop!.id,
                        userId: user.id,
                        content: Body.replace(/\[.*?\]/, '').trim(), 
                    };

                    if (!mediaUrls.length) {
                        // If no media, just save the text update
                        return db.insert(updates).values(baseUpdate).returning();
                    }

                    // If there are media attachments, create an update for each one
                    return Promise.all(
            ...
            const flattenedUpdates = savedUpdates.flat(2);
            console.log(`Saved ${flattenedUpdates.length} updates for user ${user.id}`);

            // Return a friendly response
            const response = targetLoops.length > 1
                ? `Updates shared with ${targetLoops.length} loops!`
                : `Update shared with ${targetLoops[0].loop!.name}!`;

            res.type('text/xml').send(`
          <?xml version="1.0" encoding="UTF-8"?>
          <Response>
            <Message>${response}${mediaUrls.length ? ` Including ${mediaUrls.length} media attachment${mediaUrls.length > 1 ? 's' : ''}.` : ''}</Message>
          </Response>
        `);
        } catch (error) {
